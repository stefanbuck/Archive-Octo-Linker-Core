...
var directoriesField = function($) {
  var $dirname, $dir;
  var $directories = $('.nt, .s2').filter(function(){
  ...
  if ($directories && $directories.length > 0) {
    ...
    while ($dirname) {
      ...
      if (!$dir.length) {
        $dir = $dirname.find('.s2').next('[class^="s"]');
      }
      if (!$dir || !$dir.length) {
        return;
      }
...
var dependencieField = function($, type) {
  ...
  selectors.forEach(function(selector) {
    ...
    while ($row) {
      ...
      $version = $row.find('.s2');
      if (!$item.length) {
        $item = $row.find('.s2').eq(0);
        $version = $row.find('.s2').eq(1);
      }
